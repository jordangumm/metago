...
class SampleReferenceFilter(FluxWorkflowRunner):
    """ Filters out reads that contain n-mer match to reference """
    def __init__(self, fastq, references, output_dp, max_ppn, max_mem):
        self.fastq = fastq
    ...
    def workflow(self):
        """ Sample Reference Filter Workflow """
        ...
        for ref in self.references:
            filtered_fp = os.path.join(sample_output_dp, 'filtered', '{}'.format(os.path.basename(self.fastq)))
...
class RunReferenceFilter(FluxWorkflowRunner):
